...
set dataset = ds094.0
set format  = tar
set REMOTE_HOME = https://data.rda.ucar.edu/${dataset}
...
while (${date_second_now} <= ${date_second_end})
	...
	set year_now = `date -ud ${date_now} +%Y`
	set month_now = `date -ud ${date_now} +%m`
	set day_now = `date -ud ${date_now} +%d`

	set out_path = ${OUT_HOME}/${dataset}/${year_now}/${month_now}/${day_now}
	if (! -d ${out_path}) then
		mkdir -p ${out_path}
	endif
	
	set remote_path = ${REMOTE_HOME}/${year_now}
	set filename = cdas1.${year_now}${month_now}${day_now}.pgrbh.${format}
